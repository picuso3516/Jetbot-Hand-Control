...
def get_fingers(lm):
    ...
    return fingers  # [โป้ง, ชี้, กลาง, นาง, ก้อย]
...
def get_command(hands_data):
    ...
    # มือเดียว — เช็ค backward เท่านั้น
...
# ══════════════════════════════════════════════════════════════════
# SOCKET — เชื่อมต่อ JetBot
# ══════════════════════════════════════════════════════════════════
JETBOT_IP     = "10.61.233.96"  # ← เปลี่ยนเป็น IP จริงของ JetBot
PORT          = 9999
SEND_INTERVAL = 0.5

print(f"กำลังเชื่อมต่อ JetBot {JETBOT_IP}:{PORT}...")
        ...
        while True:
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                for hand_lm, hand_info in zip(
                    ...
                    # ใช้ label ตรงๆ ไม่ swap (หลัง flip แล้วตรงกับมือผู้ใช้)
            ...
            # ── ส่ง Command ไป JetBot ─────────────────────────────
            now = time.time()
            if cmd != last_cmd or (now - last_time) > SEND_INTERVAL:
                try:
                    sock.sendall((cmd + "\n").encode())
                    last_cmd  = cmd
                    last_time = now
            ...
            cv2.imshow("JetBot Hand Gesture Control", frame)
